                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($registrations as $reg)
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-4 font-medium text-slate-900">{{ $reg->user->full_name }}</td>
                            <td class="px-6 py-4">{{ $reg->user->email }}</td>
                            <td class="px-6 py-4">{{ $reg->cnic }}</td>
                            <td class="px-6 py-4">{{ $reg->city }}</td>
                            <td class="px-6 py-4">{{ $reg->notice_period }}</td>
                            <td class="px-6 py-4">
                                {{ is_numeric($reg->area_of_expertise_id) ? optional($reg->area)->name : $reg->area_of_expertise_id }}
                            </td>
                            <td class="px-6 py-4">
                                <a href="{{ asset('storage/' . $reg->cv_path) }}" target="_blank"
                                    class="text-red-600 hover:underline text-sm">View CV</a>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.users.registrations.edit', $reg->id) }}"
                                        class="px-3 py-1 text-sm rounded-md bg-yellow-100 text-yellow-700 hover:bg-yellow-200">
                                        View & Edit
                                    </a>

                                    <form action="{{ route('admin.users.registrations.update', $reg->id) }}" method="POST"
